...
if [[ -n "${old_release_folder}" ]]; then
  ...
  echo "⚠️  Existing release folder found: ${PRIOR_RELEASE_URL}"
  read -r -p "Delete existing release folder? [y/N]: " confirm
  if [[ "${confirm}" =~ ^[Yy]$ ]]; then
    ...
    echo "ℹ️ Keeping existing release folder"
  fi
else
...
svn mv "${svn_flags[@]}" -m "Promote Apache Grails - Gradle Plugin - Grails Publish ${RELEASE_VERSION} from dev to release" "${DEV_VERSION_URL}" "${RELEASE_VERSION_URL}"
echo "✅ Promoted"
